 ...
 * Return: entry point
 */
void *load_image(void *bin, uint64_t loader_addr)
{
    int i, error;
    ...
    if ((uint64_t) hdr->e_entry == loader_addr) {
    ...
    for (i = 0; i < hdr->e_phnum; i++) {
            ...
            /* Clear memory */
            mapped_area = mmap((void *) PAGE_START(phdr[i].p_vaddr),
                            PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)),
                            PROT_READ | PROT_WRITE | PROT_EXEC,
                            MAP_PRIVATE | MAP_FIXED | MAP_ANONYMOUS, -1, 0);
            memset((void *) PAGE_START(phdr[i].p_vaddr), 0,
                            PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)));
            munmap((void*) PAGE_START(phdr[i].p_vaddr),
                            PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)));

            // page align is important!
            mapped_area = mmap((void *) PAGE_START(phdr[i].p_vaddr),
                            PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)),
                            prot, flag, fd, phdr[i].p_offset-PAGE_OFFSET(phdr[i].p_vaddr));
            if (!mapped_area) {
                fprintf(stderr, "[SEGMENTS] failed to mmap()\n");
            }
            consumed_memory += (PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)));
            if (DEBUG)
            ...
            fprintf(stderr, "[mmap(): Segments]\n\tStart address: %lx\n" 
                            "\tMapped size: %lx\n\tOffset: %lx\n\n", (uint64_t) mapped_area,
                                    PAGE_ALIGN(phdr[i].p_filesz+PAGE_OFFSET(phdr[i].p_vaddr)),
                                    phdr[i].p_offset-PAGE_OFFSET(phdr[i].p_vaddr));
            //memset(PAGE_START(phdr[i].p_vaddr), 0, PAGE_OFFSET(phdr[i].p_vaddr));
            if (is_first) {
            ...
            if (phdr[i].p_memsz > phdr[i].p_filesz) {
                    uint64_t *bss_addr = (uint64_t *) PAGE_ALIGN(phdr[i].p_vaddr + phdr[i].p_filesz);
                    unsigned long map_size = phdr[i].p_memsz - phdr[i].p_filesz;
                    void* bss_map = mmap((void *) bss_addr, map_size, prot, 
                                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0); 
                    if (!bss_map) {
                            fprintf(stderr, "[SEGMENTS] failed to mmap()\n");
                    }
                    consumed_memory += map_size;
                    if (DEBUG)
                            printf("Consumed Memory; %ld\n", consumed_memory);

                    if (DEBUG) 
                    fprintf(stderr, "[mmap(): BSS Segments]\n\tStart address: %lx\n" 
                                    "\tMapped size: %lx\n", (uint64_t) bss_map,
                                    (uint64_t) map_size);
                    memset(bss_map, 0, map_size);
            }
    }
...
void* setup_stack(void *bin, int argc, char **argv, char **envp)
{
    ...
    uint64_t stack_size = limit.rlim_cur;
    ...
    *env++;
    ...
    stack_size = (unsigned long) stack_bottom - (unsigned long) &argv[0];
...
int main(int argc, char** argv, char** envp)
{
    ...
    void *func = (void*)load_image(buf, loader_addr);
    /* Construct stack */
